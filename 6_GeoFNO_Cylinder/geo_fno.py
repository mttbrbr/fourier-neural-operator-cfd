...
class GeoFNO2d(nn.Module):
    def __init__(self, modes1, modes2, width):
        ...
        Invece di assumere una griglia cartesiana perfetta, prende in input le coordinate 
        fisiche DEFORMATE (X, Y) per mappare la geometria (il cilindro) nello spazio latente.
        """
        self.modes1 = modes1
        self.modes2 = modes2
        self.width = width

        # Input: 3 canali -> Velocità di ingresso (U_in), Coordinata fisica X, Coordinata fisica Y
        ...
        # Output: 2 canali -> Campo di velocità V_x e V_y attorno al cilindro
        self.q_out = nn.Linear(128, 2)

    def forward(self, x):
        # Il layer lineare mappa le coordinate deformate in uno spazio latente uniforme
        x = self.p(x)
        x = x.permute(0, 3, 1, 2)
